...
?> <!DOCTYPE html><html lang="<?= $locale ?>"><head><meta charset="utf-8"><title><?= $title ?></title><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,minimum-scale=1,user-scalable=no,minimal-ui"><meta name="apple-mobile-web-app-capable" content="yes"><meta name="apple-mobile-web-app-status-bar-style" content="black"><meta name="apple-mobile-web-app-title" content="PokeMap"><meta name="mobile-web-app-capable" content="yes"><meta name="theme-color" content="#3b3b3b"><link rel="shortcut icon" href="static/appicons/favicon.ico" type="image/x-icon"><link rel="apple-touch-icon" href="static/appicons/114x114.png" sizes="57x57"><link rel="apple-touch-icon" href="static/appicons/144x144.png" sizes="72x72"><link rel="apple-touch-icon" href="static/appicons/152x152.png" sizes="76x76"><link rel="apple-touch-icon" href="static/appicons/114x114.png" sizes="114x114"><link rel="apple-touch-icon" href="static/appicons/120x120.png" sizes="120x120"><link rel="apple-touch-icon" href="static/appicons/144x144.png" sizes="144x144"><link rel="apple-touch-icon" href="static/appicons/152x152.png" sizes="152x152"><link rel="apple-touch-icon" href="static/appicons/180x180.png" sizes="180x180"> <?php
    function pokemonFilterImages($noPokemonNumbers)
    {
        global $mons;
        if (empty($mons)) {
            $json = file_get_contents('static/dist/data/pokemon.min.c9da9909447accf0.json');
    ...
    ?> <script>var token = '<?php echo (!empty($_SESSION['token'])) ? $_SESSION['token'] : ""; ?>';</script><link rel="stylesheet" href="static/dist/css/app.min.cbeab92737a27cc8.css"><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.0/jquery-ui.min.css"><link rel="stylesheet" href="https://cdn.datatables.net/1.10.12/css/jquery.dataTables.css"><script src="static/js/vendor/modernizr.custom.js"></script><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/toastr.js/latest/toastr.min.css"></head><body id="top"><div class="wrapper"><header id="header"><a href="#nav"><span class="label"><?php echo i8ln('Options') ?></span></a><h1><a href="#"><?= $title ?></a></h1> <?php
        ...
        ?> <?php if (!$noWeatherOverlay) {
        ...
        } ?> <a href="#stats" id="statsToggle" class="statsNav" style="float: right;"><span class="label"><?php echo i8ln('Stats') ?></span></a></header><nav id="nav"><div id="nav-accordion"> <?php
            if (!$noPokemon) {
                ?> <h3><?php echo i8ln('Pokemon') ?></h3><div> <?php
                ...
                </div>'; ?> <div id="pokemon-filter-wrapper" style="display:none"> <?php
                        if (!$noTinyRat) {
                        ...
                        if (!$noBigKarp) {
                            ...
                            if (!$noHidePokemon) {
                                ?> <div id="tabs-1"><div class="form-control hide-select-2"><label for="exclude-pokemon"><div class="pokemon-container"><input id="exclude-pokemon" type="text" readonly="true"> <?php
                                                pokemonFilterImages($noPokemonNumbers); ?> </div><a href="#" class="select-all"><?php echo i8ln('All') ?> <div></div></a><a href="#" class="hide-all"><?php echo i8ln('None') ?></a></label></div></div> <?php
                            } ?> <?php
                            if (!$noExcludeMinIV) {
            ...
            if (!$noRaids || !$noGyms) {
                ?> <h3><?php echo i8ln('Gym/Raid'); ?></h3><div> <?php
                    ...
                    } ?> <div id="gyms-filter-wrapper" style="display:none"><div class="form-control switch-container" id="team-gyms-only-wrapper"><h3><?php echo i8ln('Team') ?></h3><select name="team-gyms-filter-switch" id="team-gyms-only-switch"><option value="0"><?php echo i8ln('All') ?></option><option value="1"><?php echo i8ln('Mystic') ?></option><option value="2"><?php echo i8ln('Valor') ?></option><option value="3"><?php echo i8ln('Instinct') ?></option></select></div><div class="form-control switch-container" id="open-gyms-only-wrapper"><h3><?php echo i8ln('Open Spot') ?></h3><div class="onoffswitch"><input id="open-gyms-only-switch" type="checkbox" name="open-gyms-only-switch" class="onoffswitch-checkbox" checked="checked"> <label class="onoffswitch-label" for="open-gyms-only-switch"><span class="switch-label" data-on="On" data-off="Off"></span> <span class="switch-handle"></span></label></div></div><div class="form-control switch-container" id="min-level-gyms-filter-wrapper"><h3><?php echo i8ln('Minimum Free Slots') ?></h3><select name="min-level-gyms-filter-switch" id="min-level-gyms-filter-switch"><option value="0">0</option><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5</option><option value="6">6</option></select></div><div class="form-control switch-container" id="max-level-gyms-filter-wrapper"><h3><?php echo i8ln('Maximum Free Slots') ?></h3><select name="max-level-gyms-filter-switch" id="max-level-gyms-filter-switch"><option value="0">0</option><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5</option><option value="6">6</option></select></div><div class="form-control switch-container" id="last-update-gyms-wrapper"><h3><?php echo i8ln('Last Scan') ?></h3><select name="last-update-gyms-switch" id="last-update-gyms-switch"><option value="0"><?php echo i8ln('All') ?></option><option value="1"><?php echo i8ln('Last Hour') ?></option><option value="6"><?php echo i8ln('Last 6 Hours') ?></option><option value="12"><?php echo i8ln('Last 12 Hours') ?></option><option value="24"><?php echo i8ln('Last 24 Hours') ?></option><option value="168"><?php echo i8ln('Last Week') ?></option></select></div></div><div id="gyms-raid-filter-wrapper" style="display:none"> <?php
    ...
    var exEligible = <?php echo $noExEligible === true ? 'false' : $exEligible  ?>;</script><script src="https://cdnjs.cloudflare.com/ajax/libs/toastr.js/latest/toastr.min.js"></script><script src="static/dist/js/map.common.min.6170e9d0715da1b3.js"></script><script src="static/dist/js/map.min.a88dd39cac40bbac.js"></script><script src="static/dist/js/stats.min.e511cef971b7dcad.js"></script><script defer="defer" src="https://maps.googleapis.com/maps/api/js?v=3.31&amp;key=<?= $gmapsKey ?>&amp;callback=initMap&amp;libraries=places,geometry"></script><script defer="defer" src="static/js/vendor/richmarker-compiled.js"></script></body></html>